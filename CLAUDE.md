...
| **Coolify: Traefik stale routing after redeploy**     | Coolify tears down and recreates all Docker Compose containers at once. Traefik's Docker provider loses track of containers during simultaneous recreation, causing all routing to hang. Deploy workflow auto-recovers by restarting `coolify-proxy` via SSH if smoke tests fail. Permanent fix: split into individual Coolify services (P1 backlog). Root cause is NOT file-based config (dynamic files only contain catchall/noop) |
