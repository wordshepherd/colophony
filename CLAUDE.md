...
| **`gh pr create` burns GraphQL rate limit**           | `gh pr create/list/view` use GraphQL (5000 pts/hr per-user, shared across all token types). ~60 pts/hr passive drain from GitHub-internal ops (not actionable). Switched `gh` auth to fine-grained PAT 2026-02-19. If large exhaustion recurs, convert skills to REST: `gh api repos/{owner}/{repo}/pulls` |
