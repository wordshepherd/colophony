...
### Schema Files (33 domain files + barrel)

`api-keys.ts`, `audit.ts`, `cms.ts`, `compliance.ts`, `contracts.ts`, `documenso.ts`, `email-templates.ts`, `embed-tokens.ts`, `enums.ts`, `federation.ts`, `forms.ts`, `hub.ts`, `identity-migrations.ts`, `issues.ts`, `manuscripts.ts`, `members.ts`, `messaging.ts`, `notifications.ts`, `notifications-inbox.ts`, `organizations.ts`, `payments.ts`, `pipeline.ts`, `publications.ts`, `relations.ts`, `saved-queue-presets.ts`, `submissions.ts`, `transfers.ts`, `trusted-peers.ts`, `user-keys.ts`, `users.ts`, `webhook-endpoints.ts`, `webhooks.ts`, `writer-workspace.ts`, `index.ts`
...
| **Custom enum arrays in raw `pg` queries**       | The `pg` driver auto-parses `text[]` but NOT custom enum arrays (e.g., `"Role"[]`). Raw `pool.query()` returns `{ADMIN,EDITOR}` as a string, not `['ADMIN','EDITOR']`. Cast to `::text[]` in SQL: `SELECT roles::text[] FROM organization_members ...`. Drizzle ORM handles this automatically; only raw `pg` queries are affected.                   |
| **`CREATE OR REPLACE` can't change return type** | PostgreSQL errors with "cannot change return type of existing function" on `CREATE OR REPLACE FUNCTION` when the return columns differ. Must `DROP FUNCTION` first, then `CREATE FUNCTION`. Affects migration 0060 (`list_user_organizations` changed from `role text` to `roles text[]`).                                                            |
