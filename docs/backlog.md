...
- [ ] [P1] Root-cause the intermittent `(dashboard)` route-group 404 — **reproduced and mitigated 2026-07-28; mechanism still open.** A CI job returns a genuine Next.js 404 (`heading "404"` / "This page could not be found.") for one or more routes under `(dashboard)` while sibling routes pass in the same run, every time passing on rerun with byte-identical content. Occurrences: `/workspace/*` 2026-07-25 (24 tests), `/federation/*` 2026-07-26 (14 of 16), `/submissions/[id]/edit` 2026-07-27 (3 tests, run 30315127919), `/submissions/[id]/edit` again 2026-07-28 (all 6 uploads tests, run 30326525136).
      **Mitigation — CI E2E now runs `next build` + `next start`.** All ten Playwright jobs build first (`pnpm --filter @colophony/web build:e2e`, ~14s) and `playwright.config.ts` serves with `next start` when `CI` is set. A production build compiles every route ahead of time, so the first-request compile boundary this failure needs does not exist. Local E2E keeps `next dev --turbo` for hot reload. **This removes the symptom from CI; it does not explain it, and it can still bite a developer locally** — hence this item stays open.
      **Reproduced locally 2026-07-28: 1 hit in 154 cold-start iterations (0.6%).** Loop: `rm -rf .next`, fresh `next dev --turbo`, then request `/` → `/submissions/<uuid>` → `/submissions/<uuid>/edit`. No DB, API or auth needed — both pages are thin server components that only `await params` and there is no `middleware.ts`, so routing resolves standalone and a bare curl status code is the whole signal. The hit:
      `✓ Ready in 268ms`
      `GET / 200 in 2.6s (next.js: 2.1s)`
      `GET /submissions/<uuid> 404 in 344ms (next.js: 309ms)` — first `(dashboard)` route requested
      `GET /submissions/<uuid>/edit 404 in 30ms (next.js: 3ms)` — negative result now cached
      Same shape as CI: the first request to a not-yet-resolved route under `(dashboard)` 404s in ~300-450ms of `next.js` time, and the path then stays 404 for the life of the process. The 0.6% rate matches four sightings across weeks of CI.
      **Correction — the "no `Compiling` line" inference was wrong, and it should not be reused.** The 2026-07-28 analysis read the absence of a `Compiling` line for the failing route as proof that Turbopack never began compiling it, and eliminated entry-compile failure, worker death and router/manifest disagreement on that basis. **Next 16 only logs `Compiling` for the first compile after startup.** Verified on 16.2.12: a cold _healthy_ run served `/` (next.js 3.6s), `/submissions/[id]` (2.7s) and `/submissions/[id]/edit` (1478ms) — all three plainly compiled — and emitted exactly one line, `○ Compiling / ...`. Worse, the _failing_ run above emitted **no `Compiling` line at all**, not even for `/`, which had just compiled in 2.1s. The line is unreliable in both directions and carries no information. The four-way cause discrimination built on it does not hold.
      What the timing still supports: 309ms of `next.js` time before the 404, against 1478ms for a successful first compile of a comparable route on a faster machine, and 3ms for the cached negative that follows. So "less than a full compile happened, then the router matched nothing" is the honest description; it does not narrow to a specific cause.
      **Leading hypothesis, untested: `✓ Ready` is emitted before route discovery has finished**, so a request arriving inside that window resolves against an incomplete route set and the miss is negatively cached. It fits the evidence — Playwright starts testing the instant the URL answers, `(dashboard)` is by far the largest subtree so it would settle last, and `/` (discovered first) never failed in 154 iterations. The one datum for it is thin: the failing run reported `Ready in 268ms` against 366ms for the healthy sample. To test it, re-run the loop with a delay inserted between `Ready` and the first request and see whether the hit rate goes to zero. Repro script is in the session scratchpad, not committed.
      Still ruled out: dependency bumps, missing/gitignored source, `notFound()` calls (none in `apps/web/src/app/`), stale `.next` cache (CI caches the pnpm store only), `reuseExistingServer` (false for web in CI), and the branch under test — 2026-07-28's PR touched only `deploy.yml`, `playwright.config.ts`, `e2e/global-setup.ts` and a package script. No shared trait across the failing routes: an async server component, a `"use client"` page, and a plain server component. The 404 carries `X-Powered-By: Next.js` with RSC `Vary` headers and no error indicator — the router matching nothing, not a 500.
      **The failing unit is an individual route, not the group or the section.** 2026-07-27 hit `/submissions/[id]/edit` on all 3 tests and both retries while `/submissions` and `/submissions/[id]` passed against the same server. Tests run sequentially in one process (`fullyParallel: false`, `workers: 1`); the failing test was the first request to that path in the process's lifetime, and tests 6-8 returned to the already-compiled detail route and passed. A whole section failing is just every leaf in it failing.
      If it recurs (locally, or in CI if the build somehow regresses): `error-context.md` in the report artifact is the quick discriminator — a 404 shows the two headings and fails identically on Playwright's in-process retry, because the route stays 404 for the life of the server; a genuine timeout re-passes. The artifact zip needs `python3 -c "import zipfile; ..."` on this box (`unzip` is not installed). Do **not** add a route pre-warming probe — it compiles the target ahead of the tests and masks the failure rather than fixing it. — (DEVLOG 2026-07-25, recurred 2026-07-26/27/28; reproduced and mitigated 2026-07-28)
...
- [x] [P1] Integration vitest configs collapsed the superuser and app pools. `vitest.config.integration-base.ts` derived `DATABASE_URL` from `DATABASE_APP_URL`, so `db`/`pool` and `appPool` from `packages/db/src/client.ts` were the same non-superuser connection for the `rls`, `webhooks`, `security` and `services` suites — every path that uses the superuser pool precisely to bypass RLS ran under RLS instead. Latent rather than breaking (the affected suites reach only `users`/`organizations`, which have no RLS, and `org-context.ts` and `stripe.webhook.ts` set the RLS context defensively before their `pool` queries), but it had surfaced three times in two sessions: fixed for `queues` alone in the outbox-poller work, worked around per-file with a `vi.mock('@colophony/db')` in `withdrawal-cascade.test.ts`, and noted again during the `listMembers` scoping. Silent because `client.ts` suppresses its `[SECURITY WARNING]` under `NODE_ENV=test` and `appPool` falls back to `DATABASE_URL`. Fixed at the base config, with three guards against recurrence — string inequality in a `setupFiles` entry, role identity in `globalSetup()`, and a unit spec that resolves every config and compares the connection identity (role, host, port, database) each one produces, which also fails on a new config not merging `integrationBase`. The spec's first version matched the config _source_ and was replaced: an intermediate `const`, a quoted property key, or two URLs differing only by a query parameter each collapse the pools while passing a source match — (2026-07-29)
...
- [ ] [P3] `pnpm sdk:generate` fails open when `openapi-python-client` is absent. `scripts/generate-sdks.ts:68-76` probes for the binary and, if missing, warns and returns — so the spec and TypeScript SDK regenerate while the Python SDK silently does not. The result is a commit that looks complete locally and fails `sdk-check` in CI on Python drift, with nothing in the local output pointing at the cause. The prerequisite is also undocumented next to the script, and Python 3.12's PEP 668 blocks `pip install --user`, so it needs a venv or pipx. Either make the skip fail loudly (exit non-zero unless an explicit `--skip-python` is passed) or document the install next to `sdk:generate` — (found 2026-07-28 while regenerating for P0.5b)
