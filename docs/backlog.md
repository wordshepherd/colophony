...
- [ ] Stripe webhook: `resourceId` passed to `insert_audit_event()` is Stripe session ID (`cs_...`), not UUID — fails `::uuid` cast in production. Either change audit schema to `varchar` for resourceId or pass a different identifier — (DEVLOG 2026-02-19, webhook integration tests)
