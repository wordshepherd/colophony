...
- [ ] [P1] Investigate the intermittent `(dashboard)` route-group 404 — **has now recurred, so this is a pattern rather than a one-off.** A CI run returns a genuine Next.js 404 (`heading "404"` / "This page could not be found." in the page snapshot) for every route under one child of `(dashboard)`, while sibling children pass in the same run. Both occurrences: `/workspace/*` 2026-07-25 (24 tests), `/federation/*` 2026-07-26 (14 of 16). Each passed on rerun with byte-identical content — 2026-07-26 failed on `main@35f9ad15` having passed on the PR head `abb2ba7f`, which is the same tree squash-merged. Ruled out: dependency bumps, missing/gitignored source, `notFound()` calls (there are none anywhere in `apps/web/src/app/`), and un-compiled-route timing (2026-07-25 trace shows the 404 eleven minutes after server start). The shared factor is the route group, not the individual section, so suspect the `(dashboard)` layout/manifest build rather than anything section-specific. `error-context.md` in the report artifact carries the page snapshot and distinguishes a 404 from a timeout — (DEVLOG 2026-07-25, recurred 2026-07-26)
