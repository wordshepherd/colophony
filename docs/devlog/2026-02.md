...
## 2026-02-28 — MigrationBundle P2 Fixes

### Done

- **decidedAt fix**: Changed `assembleBundleForUser` to use last terminal transition instead of first — handles re-decisions (e.g., REJECTED→WITHDRAWN) correctly
- **Genre JSONB validation**: Replaced unsafe `as Genre | null` cast with `genreSchema.safeParse()` — malformed DB data falls back to `null` instead of propagating invalid objects
- **Submission query limit**: Added `MAX_SUBMISSIONS = 10_000` safety cap on migration bundle query to prevent OOM for users with very large submission histories
- **Tests**: 3 new tests covering all three fixes (13 total in spec file); all 1,381 API tests passing

### Decisions

- `.limit()` placed before `.where()` in query chain — Drizzle supports this order and it avoids mock restructuring in tests
...
- **DB:** Added `REVISE_AND_RESUBMIT` to `SubmissionStatus` enum (migration 0039)
- **Shared types:** Updated Zod enum, transition maps (VALID + EDITOR_ALLOWED), status mapping (REVISE_AND_RESUBMIT ↔ "revise"), new `resubmitSchema` export
- **Service layer:** `MissingRevisionNotesError` and `NotReviseAndResubmitError` error classes; R&R comment guard in `updateStatusAsEditor()`; new `resubmitAsOwner()` method with ownership + scan validation + manuscript version swap
- **Inngest:** `hopper/submission.revise_and_resubmit` event + notification function (email + in-app + correspondence capture)
- **Email template:** `submission-revise-resubmit` with MJML + plain text (revision notes section)
- **API surfaces:** `resubmit` mutation on tRPC, REST (`POST /submissions/{id}/resubmit`), and GraphQL (`resubmitSubmission`)
- **Error mappers:** Both new error classes mapped to BAD_REQUEST across all 3 API surfaces
- **Frontend:** Amber R&R status badge; editor "Request Revisions" dialog with required revision notes; `ReviseAndResubmitCard` (writer-facing — revision notes display, manuscript version picker, resubmit button); `canWithdraw` updated to include R&R
- **Tests:** 10 transition map tests, 4 status mapping tests, 6 service access tests (R&R comment guard + resubmit ownership/status); 11 mock files updated for new error exports; all 1290 API tests + 38 types tests passing

### Decisions

- R&R is non-terminal: exits are SUBMITTED (writer resubmit) or WITHDRAWN (writer withdraw) — no editor transitions from R&R
- Editor comment required when setting R&R (enforced at service layer, not just UI)
- Resubmit validates file scan status on the new manuscript version (rejects PENDING/SCANNING/INFECTED)
...
- **Status mapping utility:** `packages/types/src/status-mapping.ts` — bidirectional `hopperToCsrStatus` / `csrToHopperStatus` with 4 test cases
- **Migration schema update:** `genre` → `genreSchema.nullable()`, `status` → `csrStatusSchema`, added `statusHistory` array and `MigrationStatusHistoryEntry` type to both history and active submission schemas
- **Bundle service enrichment:** 3 new batch-fetch blocks in `migration-bundle.service.ts` — submission periods (→ `periodName`), genre via `manuscriptVersions INNER JOIN manuscripts`, and `submission_history` (→ `decidedAt` derivation + `statusHistory` array). All conditional on data existing. Query count: 4→7 max.
- **Destination import fix:** `csrToHopperStatus()` on line 317 of `migration.service.ts` replaces unsafe cast; falls back to `'REJECTED'` for unmappable CSR statuses
- **Tests:** 4 new bundle service tests (genre population, period names, decidedAt derivation, status mapping) + updated existing tests for CSR values and new select call counts
- All 39 migration tests pass, full type-check clean (14/14)

### Decisions

- Renamed local `submissionHistory` to `closedHistory` in bundle service to avoid shadowing the DB `submissionHistory` table import
- Fallback to `'REJECTED'` for unmappable CSR statuses on destination import — these are closed submissions where the exact Hopper status is less important than having a valid terminal state
...
- **Debounce schema rebuilding:** Created shared `useDebounce` hook (`apps/web/src/hooks/use-debounce.ts`) and applied 300ms debounce to `watchedFormData` in `submission-form.tsx` and `embed-form-step.tsx` before passing to the `buildConditionalFormSchema` effect. Also consolidated the inline `useDebounce` copy in `manuscript-list.tsx` to use the shared hook.
- **Error boundaries:** Added Next.js error boundaries at dashboard (catch-all), submissions, Slate, and embed route segments — 4 new `error.tsx` files with retry + home link UI.
- **useMemo for issue-assembly:** Wrapped `sortedSections`, `unsectionedItems`, and `existingPipelineIds` in `useMemo` to avoid recalculating during every drag event.
- **Stable React keys:** Replaced index keys with `option.value` and `field.key` in `select-config.tsx` and `merge-field-manager.tsx`.
- **Skeleton constants:** Extracted inline `Array.from({ length: N })` to module-level `SKELETON_ITEMS` constants in 9 list components.
- **Dead asset cleanup:** Deleted 5 unused Next.js starter SVGs from `apps/web/public/`.
- **Tests:** 3 new tests for `useDebounce` hook. All 411 web tests pass. Type-check clean.

### Decisions

- Skipped planned Fix 7 (router.prefetch on hover) — both `SubmissionCard` and `ManuscriptCard` already use `<Link>` which Next.js prefetches in viewport; explicit hover prefetch would be redundant.
- Placed embed error boundary at `app/embed/error.tsx` instead of planned `(public)/error.tsx` — no `(public)` route group exists; public routes use `app/embed/`.
- Embed error boundary omits "Go home" link since it's an external embedding context.
...
- **PR 1 — Fix OpenAPI response schemas:** Added `.output()` Zod schemas to all 88+ procedures across 15 REST router files. Every response now has a concrete schema (was `anyOf: [{}, {not: {}}]` i.e. `unknown`). Fixed 27 test failures across 6 spec files caused by oRPC runtime output validation rejecting incomplete mock data. Re-exported spec: 67 paths, 15 tag groups, 103 concrete response schemas, 0 broken patterns.
- **TypeScript SDK (`sdks/typescript/`):** `@colophony/sdk` v0.1.0 using `openapi-fetch` + `openapi-typescript`. 8350 lines of generated types committed. `createColophonySDK()` factory with bearer/API-key auth middleware, org context header, custom fetch support.
- **Python SDK (`sdks/python/`):** `colophony` package generated by `openapi-python-client`. Importable: `from colophony import Client`.
- **Generation script (`scripts/generate-sdks.ts`):** Regenerates both SDKs from committed `sdks/openapi.json`. Added `pnpm sdk:generate` root script.
- **CI drift check:** `sdk-check` job in CI — regenerates TS SDK and fails if `git diff --quiet sdks/` detects drift.
- **OpenAPI 3.1.0 pin:** `scripts/export-openapi.ts` now writes `"openapi": "3.1.0"` for tool compatibility (openapi-python-client doesn't support 3.1.1).
- **Docs:** Updated CLAUDE.md key file locations (SDK paths) + SDK Management scripts section. Checked off SDK generation in backlog.

### Decisions

- Pinned OpenAPI to 3.1.0 (functionally identical to 3.1.1, broader tool support)
- Used `.passthrough()` on organization output schema for extra Drizzle columns
- Used `z.array(z.string())` for API key scopes (Drizzle returns string[], not enum)
- CI drift check is TS-only (Python generation requires pip install)
...
- **4 SDK adapter classes:** SmtpEmailAdapter, SendGridEmailAdapter, S3StorageAdapter, StripePaymentAdapter — all implement plugin-sdk interfaces with `configSchema` (Zod), `initialize()`, `healthCheck()`, `destroy()`
- **S3StorageAdapter multi-bucket:** SDK `StorageAdapter` methods for default bucket + extension methods (`downloadFromBucket`, `deleteFromBucket`, `uploadToBucket`, `moveBetweenBuckets`, `getSignedUrlFromBucket`) for quarantine operations
- **Config wiring:** `colophony.config.ts` maps env vars to adapter configs; `loadConfig()` from plugin-sdk creates registry; `setGlobalRegistry()` in main.ts
- **Consumer migration:** All 4 workers (email, file-scan, s3-cleanup, transfer-fetch), 3 file routers (tRPC, REST, GraphQL), 2 webhooks (Stripe, tusd), file.service, transfer.service, migration.service — all now resolve adapters from registry
- **Cleanup:** Deleted old email adapters/types, old s3.ts utility module, old email adapter tests
- **57 new test cases** across 5 new test files (SMTP 12, SendGrid 5, S3 19, Stripe 14, config 7)
- **Updated 8 existing test files** to use registry mock pattern
- **Type-check + tests green:** 13/13 type-check, 108 test files, 1190 tests

### Decisions

- Module-level registry accessor (lazy `getGlobalRegistry()`) for routers/webhooks registered at `buildApp()` time but invoked at request time; direct `AdapterRegistry` parameter for workers started after registry init
- Prefixed unused `env` params with `_` in transfer/migration services rather than removing from signatures — preserves caller API compatibility
- Arrow function → `function` keyword for vi.mock constructors — arrow functions can't be used with `new` (JavaScript spec)
...
- **Playwright E2E type-checking:** Created `apps/web/tsconfig.e2e.json` extending web tsconfig; wired into `type-check` script; added `pg`, `drizzle-orm`, `@types/pg` as web devDependencies for E2E db helpers; fixed nullable title types in E2E helpers and specs
- **Coverage tooling:** Installed `@vitest/coverage-v8` for API (Vitest); configured V8 coverage provider for web (Jest) — bypasses `minimatch@9` CJS/ESM compat issue in `test-exclude@6.0.0`/`babel-plugin-istanbul@7.0.1`
- **Coverage baseline:** API 52.3% statements / 53.4% lines (1,144 tests); Web 45.8% statements / 45.8% lines (384 tests); Slate UI components are 0% unit coverage (covered by 30 E2E tests only)

### Decisions

- V8 coverage provider for Jest over babel-plugin-istanbul: avoids `minimatch@9` breaking change (`test-exclude@6.0.0` requires `^3.0.4` but pnpm resolved `9.0.6`)
