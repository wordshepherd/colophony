...
- Upgraded `@faker-js/faker` from `^8.4.0` to `^10.4.0` (Dependabot PR #389)
- Fixed single breaking change: `faker.internet.userName()` → `faker.internet.username()` in `zitadel-fixtures.ts`
- Audited all 73 faker usages across 7 test files — no other breaking changes (code already used v8+ namespaces)
- All 1757 unit tests passing, type-check clean
