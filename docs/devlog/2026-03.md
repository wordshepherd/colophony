...
## 2026-03-16 — Dependabot Triage, ESLint 10, Architecture Review
...
- Added `.qa/` to `.gitignore` for screenshot output

### Decisions

- Dark mode via `.dark` class injection (desktop only) — app has no runtime toggle yet
- Empty states via inkwell-press org switching (localStorage) — no DB modification needed
- Error states via bogus UUID navigation (`00000000-...`) — simulates 404/error boundaries
- Viewport resize via `evaluate_script` CDP approximation — true device emulation documented as manual-only
- Section-based invocation as primary pattern for resumability
