...
## 2026-03-16 — Dependabot PR Triage + ESLint 10 Investigation
...
- Created PR #271: 46 minor/patch dep updates + removed 17 stale `eslint-disable`/`eslint-enable` directives for `@typescript-eslint/no-misused-promises` (updated typescript-eslint now correctly types Fastify preHandler) + fixed flaky notification preferences E2E test (race condition: toast appeared before switch re-rendered)
- PR #271 merged (includes fastify 5.8.2 security fix for CVE-2026-3419)
- Closed #265 (fastify-only bump, superseded by #271) and #270 (same deps without lint fix, superseded by #271)
- Investigated ESLint 9→10 upgrade: blocked by `eslint-plugin-react@7.37.5` crashing on removed `context.getSourceCode()` API, plus `eslint-plugin-react-hooks`, `eslint-plugin-jsx-a11y`, `eslint-plugin-import` all lacking ESLint 10 peer deps — all transitive via `eslint-config-next`
- Created PR #272: removed unused `eslint-plugin-import` from `packages/eslint-config` (not referenced in any active `eslint.config.mjs`)
- Created issue #273 tracking ESLint 10 upgrade with specific blockers and upstream links
- Added ESLint 10 to `docs/backlog.md` as P3 item
- Removed `@dependabot ignore` for ESLint 10 — Dependabot PRs serve as canary for when upstream plugins catch up

### Decisions

- ESLint 10 deferred until React ESLint ecosystem catches up — Dependabot canary strategy over manual polling
- `eslint-plugin-import` removed as dead weight (unused in active configs, also an ESLint 10 blocker)
