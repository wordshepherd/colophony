...
1. **Submitter role architecture:** How does the Submitter role work when someone is a submitter at one publication and an editor at another? Is this per-org role assignment, or a global identity with per-org role bindings?

2. **Self-serve org creation:** For managed hosting, can anyone create an org, or is it provisioned? For self-hosted, the deployer is presumably the admin.
