...
| **jest-dom types under pnpm**                    | `@testing-library/jest-dom/vitest` augmentation doesn't work under pnpm strict hoisting — `vitest` is unresolvable from jest-dom's pnpm store path. `vitest-matchers.d.ts` provides inline module augmentation instead. Do not remove this file or switch to importing jest-dom's `types/vitest.d.ts` |
